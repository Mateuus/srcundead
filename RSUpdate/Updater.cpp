...
void CUpdater::WaitReadyToPlay()
{
  ...
  while(true)
  {
    ::Sleep(1 * 1000);
    if(IsServerOnline()) {
      sprintf(updMsg1_, "Pressione jogar, para entrar no jogo.");
    } else {
      sprintf(updMsg1_, "Sucesso! Atualizado.");
    }
  }
  ...
  try
  {
    ...
    switch(gUserProfile.loginAnswerCode) 
    {
      ...
      case CLoginHelper::ANS_Error:
        loginErrMsg_ = "Erro ao conectar a base de dados.";
        break;

      case CLoginHelper::ANS_BadPassword:
        loginErrMsg_ = "Nome de usuário ou senha incorreto.";
        break;
      ...
      case CLoginHelper::ANS_GameActive:
        loginErrMsg_ = "Sua sessão de jogo ja está aberta.";
		Sleep(1000);
		ExitProcess(0);
        break;
      ...
      {
        loginErrMsg_ = "Sua conta está congelada";
...
void CUpdater::DoApplyNewKey(const char* key)
{
  ...
  switch(req.resultCode_)
  {
    ...
    case 2:
      MessageBox(NULL, "Your account type can not be updated, please contact support@viruzmmo.com", "Update Account", MB_OK);
      break;
