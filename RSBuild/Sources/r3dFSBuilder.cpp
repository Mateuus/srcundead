...
bool r3dFSBuilder::ReconvertAllSCO()
{
  ...
  // do a temporary scan for files, search SCOs after that
  r3dOutToLog("Scanning for .SCO and .SCB files\n");
  CLOG_INDENT;
  ...
  for(size_t i=0; i<flist_.size(); i++)
  {
    const char* filename = flist_[i].name;
	bool issco = stricmp(filename + strlen(filename) - 4, ".sco") == 0 ? true : false;
	bool isscb = stricmp(filename + strlen(filename) - 4, ".scb") == 0 ? true : false;
    if(!issco && !isscb)
      continue;
	...
	if(issco)
		filename2[strlen(filename2)-1] = 'b';
	else
		filename2[strlen(filename2)-1] = 'o';
    
    FILETIME t1, t2;
    if(!getFileTimestamp(filename, t1))
      r3dError("failed to get timestamp for %s\n", filename);

    // check for .scb
    if(!getFileTimestamp(filename2, t2)) {
      r3dOutToLog("+ %s\n", filename);
	  if(issco)
	  {
		  convertScoToBinary(filename, filename2);
		  statsco1++;
	  }
	  else
	  {
		  convertScoToBinary(filename2, filename);
		  statscb1++;
	  }
      continue;
    }
  ...
  r3dOutToLog("%d .scb converted, %d .scb reconverted\n", statscb1, statscb2);
...
bool r3dFSBuilder::CreateArchive(const char* name)
{
  ...
  char vname[MAX_PATH];
  sprintf(vname, "%s\\%s%08X", outputDir_, name, buildVersion_);
  ...
  sprintf(xmlname, "%s\\%s.xml", outputDir_, name);
  ...
  if(basefs_ != NULL)
  {
    ...
    fprintf(f, " <d ver=\"%u\" base=\"%s%s%08X\" inc=\"%s%s%08X\"", 
      buildVersion_, 
      webCDNDir_, name, basefs_->fl_.buildVersion_,
      webCDNDir_, name, buildVersion_);
    fprintf(f, "/>\n");
    fprintf(f, "</p>\n");
  }
  else
